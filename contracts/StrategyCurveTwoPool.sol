...
abstract contract StrategyCurveBase is BaseStrategy {
    ...
    BaseStrategy(_contractsAddress.vault)
  {
    ...
    want.safeApprove(_contractsAddress.gauge, type(uint256).max);
    crv.safeApprove(_contractsAddress.router, type(uint256).max);
...
contract StrategyCurveTwoPool is StrategyCurveBase {
  ...
  constructor(
    ...
    usdt.safeApprove(_contractsAddress.pool, type(uint256).max);
    usdc.safeApprove(_contractsAddress.pool, type(uint256).max);

    // Start off with USDT
    targetTokenAddress = _contractsAddress.usdt;
  }
